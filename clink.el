...
(defun clink-set-database-dir ()
  ...
  (let* ((dir (or (and clink-project-root (file-exists-p (expand-file-name clink-database-filename clink-project-root)) clink-project-root)
    ...
    (setq-local clink-project-root (expand-file-name ".clink.db" set-dir))))
...
if it is the first call, open it and return the object."
  (if-let* ((dir (expand-file-name dir))
            (db-file (expand-file-name ".clink.db" dir)))
...
(defun clink-find-symbol (symbol)
  "SYMBOL."
  (interactive (list (clink--prompt-for-symbol)))
  (let ((root-dir (file-name-directory clink-project-root))
        (plist (plist-get clink--queries-plist :symbol)))
    (clink--show-find-symbol-results
     (format "*clink-find-symbol (%s)*" symbol)
     (sqlite-select (clink--get-sqlite-database root-dir) (plist-get plist :query) (vector symbol))
...
(defun clink-setup ()
  ...
  (when-let* ((root (clink-find-project-root))
              (clink-db (expand-file-name ".clink.db" root))
